...
No webcam, no CNN, no torch. Gesture control arrives in P8, and will drive exactly the same
`request_direction` seam the keyboard uses here.
"""
...
def selftest():
    ...
    import controls as controls_module
    import game as game_module
    import ghost as ghost_module
    from controls import DELTA, Controls
    from game import (EXTRA_LIFE_SCORE, GAME_OVER, GHOST_CHAIN, MODE_SCHEDULE,
                      PELLET_SCORE, POWER_PELLET_SCORE, READY, ROUND_CLEAR, START_LIVES,
                      Game)
    from ghost import (CHASE, EATEN, FRIGHTENED, GHOST_MAX_SPEED, HOUSE, SCATTER)
    from maze import COLUMNS, GHOST_SPAWN, HOUSE_EXIT, PLAYER_SPAWN, ROWS, TILE, TUNNEL_ROW
    ...
    def play_frames(count, target=None):
    ...
    # --- maze ---------------------------------------------------------------------------
    check("maze dimensions", maze.rows == ROWS and maze.columns == COLUMNS
          and all(len(r) == COLUMNS for r in maze.layout),
          f"{maze.columns} x {maze.rows}, every row {COLUMNS} tiles")

    check("player spawn walkable", maze.is_walkable(*PLAYER_SPAWN),
          f"spawn {PLAYER_SPAWN} is a walkable tile")

    house_ok = all(maze.is_walkable(*pos, doors=True, house=True)
                   for pos in GHOST_SPAWN.values())
    check("ghost house valid", house_ok and maze.is_walkable(*HOUSE_EXIT),
          f"4 spawn tiles reachable by ghosts, exit {HOUSE_EXIT} walkable")

    check("player cannot enter house",
    ...
    check("exactly four power pellets", len(maze.power_pellets) == 4,
          f"{sorted(maze.power_pellets)}")

    check("tunnel endpoints", maze.is_walkable(TUNNEL_ROW, 0)
          and maze.is_walkable(TUNNEL_ROW, COLUMNS - 1),
          f"row {TUNNEL_ROW} open at both maze edges")

    unreachable = maze.unreachable_pellets()
    check("every pellet reachable", not unreachable,
          f"BFS from spawn reaches all {maze.pellets_remaining} pellets"
          if not unreachable else f"{len(unreachable)} unreachable: {unreachable[:6]}")

    check("no dead ends", not maze.dead_ends(),
          f"{len(maze.dead_ends())} tiles with a single exit")
    ...
    play_frames(30, "left")
    check("continuous movement", game.player.position != start,
    ...
    play_frames(20)
    check("direction request applies", game.player.direction == "right",
    ...
    check("cannot pass through walls", maze.is_walkable(*tile),
    ...
    for _ in range(60):
        ...
        if game.player.direction == "up":
            break
    check("illegal request is buffered", buffered_now,
          "UP was impossible at the moment it was requested and stayed in the buffer")
    check("buffered turn executes", game.player.direction == "up",
          f"the buffered UP fired at the first legal tile, now at {game.player.tile}")

    game.controls.clear()
    game.request_direction("down")
    game.update(step)
    check("buffer expires", game.controls.grace == controls_module.REQUEST_GRACE
          and _expires(Controls()),
    ...
    game.update(step)
    check("reversal is immediate", game.player.direction == "left",
    ...
    for index in range(300):
        ...
        if abs(now[0] - previous[0]) > 1e-6 and abs(now[1] - previous[1]) > 1e-6:
            diagonal = True
        previous = now
    check("no diagonal movement", not diagonal,
    ...
    for _ in range(120):
        ...
        if game.player.tile[1] > COLUMNS - 4:
            break
    check("tunnel wrap", game.player.tile[1] > COLUMNS - 4,
          f"walking off the left edge arrived at column {game.player.tile[1]}")

    # --- pellets and scoring --------------------------------------------------------------
    game.new_game()
    skip_ready()
    before = len(maze.pellets)
    target_tile = (22, 12)
    maze.pellets.add(target_tile)
    game.player.reset_to((22, 13), "left")
    game.score = 0
    play_frames(40, "left")
    check("pellet collected", target_tile not in maze.pellets,
          "the pellet under the player disappeared")
    check("pellet scores 10", game.score >= PELLET_SCORE and game.score % 10 == 0,
    ...
    check("power pellet scores 50", game.score >= POWER_PELLET_SCORE,
          f"score {game.score}, frightened timer "
          f"{game.frightened_timer:.1f}s started")

    frightened_count = sum(1 for g in game.ghosts if g.state == FRIGHTENED)
    check("power pellet frightens", frightened_count >= 1 and game.frightened_timer > 0,
    ...
    check("ghost chain scoring", tuple(gains) == GHOST_CHAIN,
          f"{gains} points for successive ghosts in one frightened period")

    check("eating a ghost is safe", game.lives == START_LIVES
          and all(g.state == EATEN for g in game.ghosts),
    ...
    game.update(step)
    check("round completes", game.state == ROUND_CLEAR,
          "clearing every pellet ended the round")

    lives_before, score_before, round_before = game.lives, game.score, game.round
    game.state_timer = 0.0
    game.update(step)
    check("next round starts", game.round == round_before + 1
          and game.maze.pellets_remaining > 300,
          f"round {game.round}, {game.maze.pellets_remaining} pellets refilled")
    check("round preserves progress", game.score == score_before and game.lives == lives_before,
          f"score {game.score} and {game.lives} lives carried over")

    speeds = {g.role: g.base_speed for g in game.ghosts}
    check("difficulty increases", all(v > ghost_module.GHOST_BASE_SPEED for v in speeds.values()),
          f"ghost speed now {max(speeds.values()):.2f} tiles/s "
          f"(was {ghost_module.GHOST_BASE_SPEED})")

    game.round = 40
    for ghost in game.ghosts:
        ghost.set_round(game.round)
    capped = max(g.base_speed for g in game.ghosts)
    check("difficulty capped", capped <= GHOST_MAX_SPEED < PLAYER_SPEED / TILE,
          f"ghosts cap at {capped:.2f} tiles/s, below the player's {PLAYER_SPEED / TILE:.2f}")

    frightened_late = game.frightened_duration()
    check("frightened duration floor", abs(frightened_late - 2.0) < 1e-6,
          f"round {game.round} frightened lasts {frightened_late:.1f}s (floor 2.0s)")

    game.round = 1
    check("frightened duration base", abs(game.frightened_duration() - 6.0) < 1e-6,
    ...
    game._handle_collisions()
    check("ghost collision costs a life", game.lives == START_LIVES - 1,
          f"{game.lives} lives left after touching a CHASE ghost")

    game.state_timer = 0.0
    game.update(step)
    check("pellets stay eaten after death", eaten_marker
          and (22, 12) not in game.maze.pellets,
    ...
    game.state_timer = 0.0
    game.update(step)
    check("final life ends the game", game.state == GAME_OVER,
          f"lives {game.lives}, state {game.state}")

    game.new_game()
    fresh = (game.score == 0 and game.lives == START_LIVES and game.round == 1
             and game.maze.pellets_remaining > 300 and game.state == READY
             and game.controls.pending is None
             and not game.extra_life_awarded
             and all(g.state in (HOUSE, SCATTER) for g in game.ghosts))
    check("restart resets everything", fresh,
    ...
    game.add_score(EXTRA_LIFE_SCORE)
    game.add_score(EXTRA_LIFE_SCORE)
    check("extra life at 10,000", once and game.lives == 4,
    ...
    check("four ghosts with roles", len(game.ghosts) == 4
          and roles == ["chaser", "ambusher", "flanker", "drifter"],
          ", ".join(roles))

    chaser, ambusher, flanker, drifter = game.ghosts
    game.player.reset_to((22, 13), "left")
    for ghost in game.ghosts:
        ghost.state = CHASE
    targets = {g.role: g.chase_target(game.player, chaser) for g in game.ghosts}
    check("Chaser targets the player", targets["chaser"] == game.player.tile,
          f"target {targets['chaser']} equals the player tile")
    expected_ambush = (game.player.tile[0], game.player.tile[1] - 4)
    check("Ambusher aims ahead", targets["ambusher"] == expected_ambush,
          f"target {targets['ambusher']}, four tiles ahead of a player facing left")
    check("Flanker differs from Chaser", targets["flanker"] != targets["chaser"],
    ...
    check("Drifter changes with distance",
    ...
    check("roles produce distinct targets", unique_targets >= 3,
    ...
    previous = {g.role: g.position for g in game.ghosts}
    for index in range(60 * 45):
        ...
        for ghost in game.ghosts:
            ...
            if ghost.position != previous[ghost.role]:
                moved[ghost.role] += 1
            previous[ghost.role] = ghost.position
    frozen = [role for role, count in moved.items() if count < 60]
    check("ghosts never enter walls", not intruders,
          f"45 simulated seconds, 0 wall intrusions across {len(game.ghosts)} ghosts"
          if not intruders else f"{len(intruders)} intrusions: {intruders[:4]}")
    check("no ghost gets stuck", not frozen,
          "every ghost kept moving: " +
          ", ".join(f"{r}:{c}" for r, c in moved.items()))
    ...
    staggered = len(set(round(t, 1) for t in left_times.values())) >= 3
    check("ghosts leave the house in turn", len(left_times) == 4 and staggered,
          "exit times " + ", ".join(f"{r} {t:.1f}s" for r, t in sorted(left_times.items(),
                                                                      key=lambda kv: kv[1])))
    check("house starts occupied", len(in_house_at_start) == 3,
          f"{in_house_at_start} start inside, chaser starts outside")

    # Scatter/chase schedule and the reversal on a mode switch.
    game.new_game()
    skip_ready()
    check("schedule starts in scatter", game.mode == SCATTER,
    ...
    check("scatter -> chase switch", game.mode == CHASE,
          f"mode advanced to {game.mode} when the phase timer expired")
    check("ghosts reverse on mode change", reversed_any,
    ...
    check("frightened mode entered", ghost.state == FRIGHTENED
          and ghost.current_speed() < ghost.base_speed * TILE,
          f"speed drops to {ghost.current_speed() / TILE:.1f} tiles/s while edible")

    ghost.get_eaten()
    check("eaten ghost speeds up", ghost.state == EATEN
          and ghost.current_speed() > ghost.base_speed * TILE,
    ...
    for _ in range(60 * 12):
        ...
        if ghost.state in (HOUSE, SCATTER, CHASE):
            returned = True
            break
    check("eaten ghost returns home", returned,
    ...
    game._handle_collisions()
    check("eaten ghosts are harmless", game.lives == lives_before,
    ...
    check("keyboard uses the generic seam", keyboard_request == "right",
          "a key press only produced a direction request, it did not move anything")
    check("programmatic control is identical", programmatic == keyboard == "right",
    ...
    for direction in DELTA:
        ...
        if fresh_controls.pending != direction:
            seam_ok = False
    check("all four directions requestable", seam_ok,
    ...
    banned_modules = {"torch", "torchvision", "cv2", "numpy.distutils",
                      "gesture_recognizer", "realtime_gesture", "data_pipeline",
    ...
    for name in sorted(os.listdir(package)):
        if not name.endswith(".py"):
            continue
        scanned.append(name)
        tree = ast.parse(open(os.path.join(package, name), encoding="utf-8").read())
    ...
    check("no CNN or webcam imports", not offenders,
          f"{len(scanned)} modules ({', '.join(scanned)}) import none of "
          f"{sorted(banned_modules)}" if not offenders else f"{offenders}")
    ...
    check("CNN stack never loaded", not loaded,
          "torch, torchvision and cv2 are absent from sys.modules after running the "
          "whole game and its tests" if not loaded else f"loaded: {sorted(loaded)}")
...
def fps_probe(frames):
    ...
    game = Game(headless=True)
    game.state = __import__("game").PLAYING
    step = 1.0 / 60.0
