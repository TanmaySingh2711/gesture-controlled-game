...
from game import DYING, GAME_OVER, PLAYING, ROUND_CLEAR, Game
from game_integration import (STALE_SECONDS, GestureController, RecognitionWorker,
...
def fake(state, command, now, sequence=1, **extra):
    ...
    state.publish(sequence=sequence, timestamp=now, stable_command=command,
                  raw_direction=command or "up", raw_confidence=0.99,
                  thresholded_direction=command, camera_ok=True, status="ready",
                  inference_ms=12.5, **extra)
...
def main():
    ...
    for index in range(60):                       # a full second of "no command"
        ...
        if applied is not None:
            break
    check("None issues no request", applied is None,
          "sixty consecutive None snapshots produced no direction request")
    check("None does not stop the player", game.player.direction == direction_before
          and game.player.position != moved_before,
          f"Pac-Man kept travelling {direction_before} through a second of no-command frames")
    check("None clears the buffer", game.controls.pending is None,
    ...
    for index in range(40):
        ...
        if controller.apply_to(game, now):
            refreshed += 1
        game.update(step)
    check("held gesture refreshes intent", refreshed >= 35,
    ...
    for index in range(120):
        ...
        if game.player.direction == "up" and turned_at is None:
            turned_at = index
            break
    check("buffered pre-turn works", turned_at is not None,
    ...
    check("stale results expire", fresh_applied == "left" and stale_applied is None,
          f"a snapshot older than {STALE_SECONDS:.2f}s stopped being applied")

    snapshot, fresh = controller.poll(now + STALE_SECONDS + 0.05)
    check("stale shows as waiting", not fresh
          and controller.status_line(now + STALE_SECONDS + 0.05) == "GESTURE: WAITING",
    ...
    controller.apply_to(game, now)
    state.publish(camera_ok=False, status="camera error", error="device lost",
                  stable_command=None)
    applied = controller.apply_to(game, now)
    line = controller.status_line(now)
    game.request_direction("left")                 # keyboard must still work
    check("camera failure stops commands", applied is None,
          f"status line reads {line!r}")
    check("keyboard survives camera failure", game.controls.pending == "left",
    ...
    controller.apply_to(game, now)
    check("new gesture replaces the old", first == "up" and game.controls.pending == "down",
    ...
    plan = [("left", 60), (None, 30), ("up", 60), (None, 30), ("right", 60)]
    for command, frames in plan:
        for index in range(frames):
            now += step
            game_frames += 1
            if index % 2 == 0:                      # recognizer publishes at half the rate
                published += 1
                fake(state, command, now, sequence=published)
            if controller.apply_to(game, now):
                applied_total += 1
            game.update(step)
    backlog = state.writes - published              # one slot: writes must equal publishes
    check("30 FPS recognizer, 60 FPS game", game_frames == 240 and published == 120,
          f"{game_frames} game frames consumed {published} recognition updates")
    check("no backlog accumulates", backlog == 0,
          "the shared slot holds exactly one snapshot; writes == publishes, nothing queues")
    check("commands survive the rate gap", applied_total > 150,
    ...
    controller.reset()
    check("reset clears integration state", had_command == "left"
          and controller.last_applied is None and controller.last_sequence == 0,
          "the held direction and sequence marker are dropped on reset")

    game.request_direction("up")
    game._reset_positions()
    check("life reset clears the buffer", game.controls.pending is None,
          "the game's own reset already empties the request buffer on death and round change")

    game.new_game()
    check("restart clears the buffer", game.controls.pending is None
          and game.score == 0 and game.lives == 3,
    ...
    check("gesture uses the P7 seam", gesture_pending == keyboard_pending == "right",
    ...
    check("most recent request wins", game.controls.pending == "down",
    ...
    app.controller = GestureController(SharedState())
    resets = []
    app.controller.reset = lambda: resets.append(app.game.state)
    ...
    app.handle_events()
    check("pauses clear control state", not during_play and paused == [DYING, ROUND_CLEAR],
          "state is dropped on death and round change, left alone during normal play")
    check("restart clears control state", len(resets) == 3 and app.game.state != GAME_OVER,
    ...
    check("worker shuts down cleanly", released and not worker.is_alive() and published > 0,
          f"a worker that published {published} snapshots stopped and joined on request")
    check("shutdown issues no command", after is None and state.read().status == "stopped",
          "once stopped the state reads 'stopped' and steers nothing")
    check("stop flag does not shadow Thread", callable(getattr(worker, "_stop", None)),
          "the flag is _stop_event, so Thread._stop stays the method join() relies on")

    idle = RecognitionWorker(SharedState())
    check("stop before start is safe", GestureController(idle.state, idle).stop() is True,
    ...
    game.request_direction("up")
    check("camera error survives exit", line == "CAMERA ERROR - keyboard still works"
          and applied is None and game.controls.pending == "up",
    ...
    touches_physics = any(word in source for word in
                          (".x =", ".y =", ".direction =", "player.tile", "_advance"))
    check("no duplicated physics", not touches_physics,
    ...
    check("game package still CNN-free", not leaked,
