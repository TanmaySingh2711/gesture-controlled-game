...
def check_mapping(split: dict[str, Any]) -> None:
    ...
    record(
        "Mapping constant",
        CLASS_TO_INDEX == REQUIRED_MAPPING,
        f"{CLASS_TO_INDEX}"
        if CLASS_TO_INDEX == REQUIRED_MAPPING
    ...
    record(
        "class_mapping.json",
        consistent,
        "four classes, indices 0-3, forward and reverse maps agree"
        if consistent
        else f"file disagrees with the code: {from_file}",
    )

    retired = sorted(
        {"jump", "neutral", "peace", "no_gesture"}
        & (set(from_file) | set(reverse.values()) | set(CLASS_TO_INDEX))
    )
    record(
        "No retired classes",
        not retired,
        "no jump/neutral/peace/no_gesture anywhere in the mapping"
        if not retired
    ...
    record(
        ...
        "data_splits.json carries the same mapping"
        if split["class_to_index"] == REQUIRED_MAPPING
...
def check_transform_safety() -> None:
    ...
    offenders = [n for n in names if n.lower() in BANNED_TRANSFORMS or "vertical" in n.lower()]
    record(
        "No vertical flip",
        not offenders,
        "no vertical flip or orientation-reversing transform in the training pipeline"
        if not offenders
        else f"forbidden transform(s): {', '.join(sorted(set(offenders)))}",
    )
    ...
    record(
        "Rotation bounded",
        bounded,
        f"RandomAffine rotates at most +/-{max(degrees):.0f} degrees (limit {MAX_SAFE_ROTATION})"
        if affine
    ...
    record(
        ...
        not stochastic,
        f"deterministic only: {' -> '.join(evaluation)}"
        if not stochastic
...
def check_every_class(split: dict[str, Any]) -> None:
    ...
    for name in CLASSES:
        ...
        good = (
            tuple(train_tensor.shape) == (3, IMAGE_SIZE, IMAGE_SIZE)
...
def check_loader_config(loaders: dict[str, DataLoader[Any]], batch_size: int, workers: int) -> None:
    ...
    record("Loader shuffling", all(shuffles), "train shuffled, validation and test in fixed order")
...
def check_split(split: dict[str, Any]) -> None:
    ...
    for name, expected_count in (
        ...
        if any(tally[c] != expected_count for c in CLASSES):
            per_class_ok = False
    record("Per-class split", per_class_ok, " ".join(detail) + f"  (order {'/'.join(CLASSES)})")

    paths = {name: {entry["path"] for entry in items} for name, items in split["splits"].items()}
    overlaps = []
    ...
    record(
        "Split overlap",
        not overlaps,
        "no image appears in two splits" if not overlaps else ", ".join(overlaps),
    )
    ...
    labels_ok = all(
        entry["label"] == CLASS_TO_INDEX[entry["class"]]
        for items in split["splits"].values()
        for entry in items
    )
    record(
        "Label mapping",
        labels_ok,
        "left=0, right=1, up=2, down=3 applied consistently"
        if labels_ok
...
def check_batches(loaders: dict[str, DataLoader[Any]], train_batches: int = 3) -> None:
    ...
    for name, loader in loaders.items():
        ...
        shape_ok = all(
            b[0].ndim == 4
            and b[0].shape[1] == 3
            and b[0].shape[2] == IMAGE_SIZE
            and b[0].shape[3] == IMAGE_SIZE
            and b[1].shape[0] == b[0].shape[0]
            for b in seen
        )
        record(
            f"{name} batch shape",
        ...
        labels_ok = all(
            bool(b[1].dtype in (torch.int64, torch.int32) and b[1].min() >= 0 and b[1].max() <= 3)
            for b in seen
        )
...
def check_determinism(split: dict[str, Any]) -> None:
    ...
    record(
        "Eval determinism",
        identical,
        "validation preprocessing is bit-identical across loads"
        if identical
    ...
    record(
        "Train augmentation live",
        varies,
        "training samples differ between loads, as expected"
        if varies
...
def write_augmentation_grid(split: dict[str, Any], variants: int = 6) -> None:
    ...
    for name in CLASSES:
        ...
        original = cv2.resize(loaded, (IMAGE_SIZE, IMAGE_SIZE))
        cv2.putText(
            original, "original", (6, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1
        )
        cells = [original]
...
def write_updown_grid(split: dict[str, Any], rows_per_class: int = 3, variants: int = 7) -> None:
    ...
    for name in ("up", "down"):
        ...
        cv2.putText(
            header,
            f"{name.upper()}  original + {variants} augmented   "
...
def main() -> int:
    ...
    print(
        f"input {IMAGE_SIZE}x{IMAGE_SIZE} | batch {args.batch_size} | "
