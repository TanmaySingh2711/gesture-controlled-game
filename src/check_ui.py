...
from game import DYING, GAME_OVER, PLAYING, READY, ROUND_CLEAR, WINDOW_HEIGHT, WINDOW_WIDTH, Game
from game_integration import STALE_SECONDS, GestureController, SharedState
from play_gesture import (GESTURE_ROWS, KEYBOARD_ROWS, PANEL_WIDTH, TITLE, GesturePacman,
                          command_text, confidence_text, control_status, loading_text)
...
def publish(state, command, now, sequence=1, **extra):
    state.publish(sequence=sequence, timestamp=now, stable_command=command,
                  raw_direction=command or "up", raw_confidence=0.986,
                  thresholded_direction=command, camera_ok=True, status="ready",
                  inference_ms=16.4, **extra)
...
def main():
    ...
    check("start screen opens first", on_start and by_space and app2.phase == "playing",
          "the app opens on the start screen; SPACE and ENTER both begin play")

    app3, _ = wired()
    frozen = app3.game.player.position
    app3.draw_start()
    check("start screen does not play", app3.game.player.position == frozen
          and app3.game.state == READY,
    ...
    check("loading states read clearly",
          loading == "Initializing gesture recognition..." and ready_line == "Camera ready"
    ...
    for index, command in enumerate(("left", "right", "up", "down"), start=1):
        ...
        app.draw_panel()
    check("four commands display", shown == ["LEFT", "RIGHT", "UP", "DOWN"],
          ", ".join(shown))
    ...
    check("None reads as NO COMMAND", none_text == "NO COMMAND" and none_conf is None,
    ...
    check("stale reads as WAITING", waiting == "WAITING"
    ...
    check("camera error displays", error_text == "CAMERA ERROR"
          and error_status == "CAMERA ERROR",
          "the panel shows CAMERA ERROR and renders its keyboard advice without overflow")

    off_app, off_state = wired(use_camera=False)
    snapshot, fresh = off_app.controller.poll()
    check("keyboard-only reads OFF", command_text(snapshot, fresh, False) == "GESTURE OFF"
    ...
    snapshot, fresh = app.controller.poll()
    check("confidence shown for a command", confidence_text(snapshot, fresh, True)
          == "Confidence: 98.6%",
    ...
    check("score, lives and round display", values["score"].strip() == "SCORE   1230"
          and values["lives"] == 2 and values["round"] == "ROUND 3",
          f"{values['score'].strip()} | LIVES {values['lives']} | {values['round']}")

    game.score = 0
    game.lives = 3
    game.round = 1
    check("HUD only reports state", game.hud_text()["score"].strip() == "SCORE      0",
    ...
    check("READY and LIFE LOST read clearly", ready_banner == ("READY!", [])
          and dying_banner == ("LIFE LOST", ["Lives Remaining: 2"]),
          "the ready pause and the death pause both explain themselves")
    check("round clear displays", round_banner[0] == "ROUND 2 CLEARED!",
          f"{round_banner[0]!r}")
    check("game over displays fully", over_banner[0] == "GAME OVER"
    ...
    check("restart instructions display", "Press R to Restart" in over_banner[1]
          and "Press ESC to Quit" in over_banner[1],
          "the player is told how to restart and how to quit")
    check("no banner during play", playing_banner is None,
    ...
    app.draw_start()
    check("every screen renders", len(drawn) == 5,
          "start screen and all five game states draw without raising")

    # --- the panel never covers the maze --------------------------------------------------
    surface = app.game.screen
    check("panel does not cover the maze",
          surface.get_width() == WINDOW_WIDTH + PANEL_WIDTH
          and surface.get_height() == WINDOW_HEIGHT,
    ...
    same = (drawn_app.game.player.position == plain.player.position
    ...
    check("UI does not change physics", same,
    ...
    check("drawing stays cheap", per_frame < 8.0,
