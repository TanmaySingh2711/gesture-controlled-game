...
from controls import REQUEST_GRACE
from game import (DYING, EXTRA_LIFE_SCORE, GAME_OVER, GHOST_CHAIN, PELLET_SCORE, PLAYING,
                  POWER_PELLET_SCORE, READY, ROUND_CLEAR, START_LIVES, Game)
from game_integration import (GestureController, RecognitionWorker, SharedState, Snapshot)
from ghost import CHASE, EATEN, FRIGHTENED, HOUSE, SCATTER
from maze import TUNNEL_ROW
from play_gesture import GesturePacman, command_text, control_status
...
KEYBOARD_PROBE = ("import sys; sys.path.insert(0, 'src'); sys.path.insert(0, 'game'); "
                  "from play_gesture import GesturePacman; "
...
def publish(state, command, now=None, sequence=1):
    state.publish(sequence=sequence, timestamp=now if now is not None else time.perf_counter(),
                  stable_command=command, raw_direction=command or "up", raw_confidence=0.97,
                  thresholded_direction=command, camera_ok=True, status="ready",
                  inference_ms=16.0)
...
def main():
    ...
    from data_pipeline import CLASS_TO_INDEX
    payload = torch.load(CHECKPOINT, map_location="cpu", weights_only=False)
    mapping = payload.get("class_to_index")
    check("final checkpoint identity",
    ...
    check("frozen class mapping", mapping == CLASS_TO_INDEX == {"left": 0, "right": 1,
                                                                "up": 2, "down": 3},
    ...
    check("frozen recognition settings",
    ...
    check("application starts on the start screen",
    ...
    check("one worker, started once", started == 1 and released,
    ...
    check("gesture drives the player", game.player.direction == "right"
          and game.player.tile != start_tile,
    ...
    check("None does not stop Pac-Man", game.player.direction == "left"
          and game.player.position != moving and game.controls.pending is None
    ...
    score_before = game.score
    game.update(1.0 / 60.0)
    check("a frightened ghost can be eaten", victim.state == EATEN
          and game.score == score_before + GHOST_CHAIN[0],
    ...
    for _ in range(120):                          # let the death pause run out
        game.update(1.0 / 60.0)
    check("life loss costs exactly one life", lost_one and game.state == READY,
          f"lives {START_LIVES} -> {game.lives}, then the round resumes at READY")
    check("the board survives a death", game.score == score_kept
          and game.maze.pellets_remaining == pellets_kept,
          f"score {game.score} and {game.maze.pellets_remaining} remaining pellets both "
          "carried through the death")

    controller.reset()
    check("respawn is not steered by a stale gesture",
    ...
    check("Game Over ends the game", game.state == GAME_OVER and headline == "GAME OVER"
          and f"Final Score: {game.score}" in details
          and f"Round Reached: {game.round}" in details
          and "Press R to Restart" in details and "Press ESC to Quit" in details,
    ...
    for _ in range(60):
        game.update(1.0 / 60.0)
    check("Game Over stops play", game.player.position == moved,
          "nothing moves once the game is over; it waits for R")

    game.handle_event(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_r))
    controller.reset()
    fresh = (game.state == READY and game.score == 0 and game.lives == START_LIVES
             and game.round == 1 and game.controls.pending is None)
    game.state, game.state_timer = PLAYING, 0.0
    sequence = gesture_run(state, controller, game, "left", 60)
    check("restart gives a clean game", fresh and game.maze.pellets_remaining > 300,
          f"score 0, {START_LIVES} lives, round 1, "
          f"{game.maze.pellets_remaining} pellets back on the board")
    check("gestures still steer after restart", game.player.direction == "left",
    ...
    check("round completes and advances", cleared and game.round == 2
          and game.state == READY and game.maze.pellets_remaining > 300,
          f"the last pellet cleared round 1; round {game.round} starts with "
          f"{game.maze.pellets_remaining} pellets restored")
    check("progress carries into the round", game.score >= 500 + PELLET_SCORE
          and game.lives == 2 and speeds_after > speeds_before,
    ...
    check("the tunnel wraps under gesture control", wrapped[0] == TUNNEL_ROW
          and wrapped[1] > 20,
    ...
    check("extra life is awarded once", after_first == START_LIVES + 1
          and game.lives == after_first,
    ...
    for _ in range(30):
        game.update(1.0 / 60.0)
    check("keyboard and gesture share one seam", keyboard_won and gesture_won
          and game.player.position != position,
    ...
    check("keyboard-only mode reads OFF", off.controller is None
    ...
    check("keyboard-only launch loads no CNN",
    ...
    for _ in range(60):
        game.update(1.0 / 60.0)
    check("camera failure degrades to keyboard", applied is None
        ...
        def run(self):
            try:
                ...
                while not self._stop_event.is_set():
                    self.frames += 1
                    self.state.publish(sequence=self.frames, timestamp=time.perf_counter(),
                                       stable_command="up", camera_ok=True, status="ready")
                    time.sleep(0.002)
            finally:
                self._publish_exit()
    ...
    check("shutdown is clean and complete", released and not worker.is_alive()
          and published > 0 and after is None
    ...
    check("checkpoint untouched by testing",
