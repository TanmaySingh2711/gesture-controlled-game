...
from gesture_recognizer import (DEFAULT_MIN_AGREEMENT, DEFAULT_THRESHOLD, DEFAULT_WINDOW,
                                DIRECTION_HELP, DIRECTIONS, FRAME_HEIGHT, FRAME_WIDTH,
                                NO_COMMAND_LABEL, PROJECT_ROOT, ROI_X1, ROI_X2, ROI_Y1,
                                ROI_Y2, DirectionRecognizer, label_of, open_camera)
...
TRANSITION_PAIRS = [("left", "right"), ("right", "up"), ("up", "down"),
                    ("down", "left"), ("left", "up"), ("right", "down")]

COLOR = {"left": (80, 200, 255), "right": (120, 220, 120),
         "up": (255, 190, 90), "down": (200, 140, 255), None: (170, 170, 170)}
...
def draw_overlay(frame, result, fps, recognizer, show_raw, banner=None):
    ...
    cv2.putText(frame, "keep the whole hand inside this box", (ROI_X1 - 4, ROI_Y1 - 8),
                cv2.FONT_HERSHEY_SIMPLEX, 0.42, colour, 1)

    cv2.rectangle(frame, (0, 0), (FRAME_WIDTH, 100), (0, 0, 0), -1)
    cv2.putText(frame, f"RAW: {result.raw_direction.upper()} "
                       f"{result.raw_confidence * 100:.1f}%", (10, 26),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    thresholded = (label_of(result.thresholded_direction)
                   if result.thresholded_direction else "REJECTED")
    cv2.putText(frame, f"THRESHOLDED: {thresholded}", (10, 52),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    cv2.putText(frame, f"STABLE: {label_of(stable)}", (10, 86),
                cv2.FONT_HERSHEY_SIMPLEX, 0.85, colour, 2)

    cv2.putText(frame, f"Threshold: {recognizer.threshold:.2f}", (400, 26),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (170, 170, 170), 1)
    cv2.putText(frame, f"Smoothing: {recognizer.min_agreement}-of-{recognizer.window}",
                (400, 48), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (170, 170, 170), 1)
    cv2.putText(frame, f"CNN: {result.inference_ms:4.1f} ms", (400, 70),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (170, 170, 170), 1)
    cv2.putText(frame, f"FPS: {fps:5.1f}", (400, 92),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (170, 170, 170), 1)

    if show_raw:
        window_text = " ".join("-" if item is None else item[0].upper()
                               for item in recognizer.history)
        cv2.putText(frame, f"window [{window_text}]", (10, FRAME_HEIGHT - 92),
    ...
    cv2.putText(frame, legend, (10, FRAME_HEIGHT - 52),
                cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200, 200, 200), 1)
    cv2.putText(frame, "hand should fill most of the box  |  Q quit  R reset  D debug",
                (10, FRAME_HEIGHT - 28), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (160, 160, 160), 1)

    if banner:
        cv2.rectangle(frame, (0, FRAME_HEIGHT - 112), (FRAME_WIDTH, FRAME_HEIGHT - 80),
                      (30, 30, 30), -1)
        cv2.putText(frame, banner, (10, FRAME_HEIGHT - 91),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.52, (90, 230, 255), 1)
    return frame
...
def preview(recognizer):
    ...
    if latencies:
        print(f"frames {len(latencies)} | mean CNN {np.mean(latencies):.2f} ms | "
...
def benchmark(recognizer, frames, show):
    ...
    print(f"loop time  mean/p95   : {np.mean(loop_times):.2f} / "
    ...
    print(f"smoothing cost        : {recognizer.min_agreement} frames = "
          f"{recognizer.min_agreement * frame_ms:.0f} ms at this frame rate")
    return {
        "frames": len(latencies), "fps": fps, "frame_ms": frame_ms,
...
def trials(recognizer, category_list, count, out_path):
    ...
    For the four directions the correct answer is that direction. For no_hand and idle_hand
    the correct answer is NO COMMAND - those categories measure whether the recognizer stays
    quiet when it should, which matters more than raw accuracy for a model with no reject
    class.
    """
    capture = open_camera()
    meter = FrameRate()
    ...
    print(f"Recording {len(plan)} trials ({count} per category: "
    ...
    try:
        while position < len(plan):
            ...
            wanted = (NO_COMMAND_LABEL if expected in NO_COMMAND_CATEGORIES
                      else expected.upper())
            banner = (f"TRIAL {position + 1}/{len(plan)}  -  {CATEGORY_HELP[expected]}"
            ...
            if key == ord(" "):
                ...
                print(f"  [{mark}] {position + 1:>3}/{len(plan)}  {expected:<10} "
                      f"raw {result.raw_direction:<6} {result.raw_confidence:.3f}  "
                      f"stable {label_of(result.stable_command)}")
                position += 1
                recognizer.reset()
            if cv2.getWindowProperty(window, cv2.WND_PROP_VISIBLE) < 1:
                break
...
def summarise(records, category_list):
    ...
    for name in category_list:
        ...
        print(f"  {name:<10} {hits}/{len(subset)} ({hits / len(subset) * 100:5.1f}%)  "
              f"raw conf mean {np.mean(confidences):.3f} min {min(confidences):.3f}"
              f"{detail}")

    # For the no-command categories, the headline number is how often a false command
    # actually survived smoothing - that is what would turn Pac-Man the wrong way.
    ...
    if quiet:
        ...
        print(f"  stable false commands: {len(false_commands)}/{len(quiet)} "
              f"({len(false_commands) / len(quiet) * 100:.1f}%)")
        if false_commands:
            print("    " + ", ".join(f"{r['expected_class']}->{r['stable_command']} "
                                     f"({r['raw_confidence']:.3f})"
                                     for r in false_commands))

    # Raw mistakes that smoothing caught are the direct evidence that smoothing earns its
    # place, which P5 predicted it would.
    ...
    if rescued:
        ...
        for r in rescued:
            print(f"    trial {r['trial_number']}: raw {r['raw_prediction']} "
...
def transitions(recognizer, repeats, out_path):
    ...
    Measures how long the intended command takes to stabilise, and whether any *other*
    stable command appears in between. A brief NO COMMAND during the movement is fine and
    expected; a confident wrong direction is not.
    """
    capture = open_camera()
    meter = FrameRate()
    ...
    print(f"{len(plan)} transitions. Hold the FROM gesture, then press SPACE at the moment "
    ...
    try:
        while position < len(plan):
            ...
            if timing is not None:
                timing["frames"] += 1
                # The moment the hand first *looks* like the target to the CNN. Everything
                # before this is the human moving; everything after is the recognizer
                # deciding. Only the second part is the recognizer's own latency.
                if timing["raw_seen"] is None and result.raw_direction == target:
                    timing["raw_seen"] = time.perf_counter()
                    timing["raw_frames"] = timing["frames"]
                if result.stable_changed and result.stable_command is not None:
                    if result.stable_command == target:
                        timing["done"] = True
                    elif result.stable_command != source:
                        timing["spurious"].append(result.stable_command)
                if timing["done"]:
                    now = time.perf_counter()
                    elapsed = (now - timing["started"]) * 1000.0
                    if timing["raw_seen"] is not None:
                        lag_ms = (now - timing["raw_seen"]) * 1000.0
                        lag_frames = timing["frames"] - timing["raw_frames"] + 1
                    else:
                        lag_ms, lag_frames = float("nan"), 0
                    records.append({
                        "transition": f"{source}->{target}",
                        "from_direction": source,
                        "to_direction": target,
                        "frames_to_stable": timing["frames"],
                        "ms_to_stable": round(elapsed, 1),
                        "recognizer_lag_frames": lag_frames,
                        "recognizer_lag_ms": round(lag_ms, 1),
                        "spurious_commands": "|".join(timing["spurious"]) or "none",
                        "spurious_count": len(timing["spurious"]),
                        "threshold": recognizer.threshold,
                        "smoothing": f"{recognizer.min_agreement}-of-{recognizer.window}",
                        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                    })
                    mark = "OK " if not timing["spurious"] else "SPUR"
                    print(f"  [{mark}] {position + 1:>3}/{len(plan)}  {source}->{target}  "
                          f"total {elapsed:.0f} ms  |  recognizer lag {lag_ms:.0f} ms "
                          f"({lag_frames} frames)"
                          + (f"  spurious: {timing['spurious']}"
                             if timing["spurious"] else ""))
                    timing = None
                    position += 1
                    recognizer.reset()
                    continue

            state = "MOVING - waiting for stable" if timing else "hold FROM, SPACE to start"
            banner = (f"{position + 1}/{len(plan)}  {source.upper()} -> {target.upper()}"
                      f"   [{state}]   S=skip")
            draw_overlay(frame, result, meter.tick(), recognizer, True, banner)
            cv2.imshow(window, frame)
            ...
            if key == ord("s"):
                ...
                recognizer.reset()
                continue
            if key == ord(" ") and timing is None:
                timing = {"started": time.perf_counter(), "frames": 0, "spurious": [],
                          "done": False, "raw_seen": None, "raw_frames": 0}
            if cv2.getWindowProperty(window, cv2.WND_PROP_VISIBLE) < 1:
                break
    finally:
        capture.release()
    ...
    lags = [r["recognizer_lag_ms"] for r in records
            if r["recognizer_lag_frames"]]
    spurious = [r for r in records if r["spurious_count"]]
    print("-" * 72)
    print(f"transitions recorded  : {len(records)}")
    print(f"total move+settle     : mean {np.mean(times):.0f} ms | "
          f"median {np.median(times):.0f} ms   (includes your hand movement)")
    if lags:
        print(f"RECOGNIZER LAG        : mean {np.mean(lags):.0f} ms | "
              f"median {np.median(lags):.0f} ms | p95 {np.percentile(lags, 95):.0f} ms")
        print(f"  (raw prediction first matches the target -> stable command issued; "
              f"this is the part the game feels)")
...
def selftest(recognizer):
    ...
    from data_pipeline import CLASS_TO_INDEX, eval_transform
    ...
    check("checkpoint", meta.get("task") == "pacman_direction_v1",
          f"{meta['architecture']}, task {meta.get('task')}, "
          f"input {meta['input_size']}")
    check("class mapping", meta["class_to_index"] == CLASS_TO_INDEX,
          f"{meta['class_to_index']}")
    check("no retired classes",
    ...
    if os.path.exists(obsolete):
        try:
            ...
            check("obsolete rejected", "Refusing to load" in str(error),
                  "guard refused the endless-runner checkpoint")
    else:
        check("obsolete rejected", True, "obsolete checkpoint not present")

    # Preprocessing parity: the webcam path must build the same tensor as the P5 eval path.
    ...
    check("preprocessing", identical,
          f"identical to the P3/P5 eval transform (max delta {delta:.2e})")

    # --- smoothing semantics -------------------------------------------------------------
    recognizer.reset()
    stable = [recognizer._smooth(d) for d in ["up", "up", None, "up", "up"]]
    check("smoothing majority", stable[-1] == "up",
          f"['up','up',None,'up','up'] -> {label_of(stable[-1])}")

    recognizer.reset()
    flicker = [recognizer._smooth(d) for d in ["up", "down", "left", "down", "up"]]
    check("no majority", flicker[-1] is None,
          f"flickering directions -> {label_of(flicker[-1])}")

    recognizer.reset()
    rejected = [recognizer._smooth(None) for _ in range(5)]
    check("rejections stay quiet", all(s is None for s in rejected),
    ...
    check("command can clear", cleared[-1] is None,
    ...
    for _ in range(recognizer.window):
        ...
        if recognizer._smooth("left") == "left":
            break
    check("stabilisation cost", needed == recognizer.min_agreement,
...
def main():
    ...
    parser.add_argument("--idle", action="store_true",
                        help="no-hand and idle-hand recorder (expects NO COMMAND)")
    parser.add_argument("--transitions", action="store_true",
                        help="transition stress test and stabilisation latency")
    parser.add_argument("--repeats", type=int, default=2,
                        help="repeats of the 6 transition pairs for --transitions")
    parser.add_argument("--classes", default=None,
                        help="comma-separated categories for --trials")
    parser.add_argument("--count", type=int, default=TRIALS_PER_CLASS,
                        help="trials per category")
    ...
    recognizer = DirectionRecognizer(threshold=args.threshold, window=args.window,
                                     min_agreement=args.agreement)
    print(f"checkpoint {os.path.basename(recognizer.metadata.get('task', 'unknown'))} | "
    ...
    if args.idle:
        ...
        return 0
    if args.trials:
        selected = ([c.strip() for c in args.classes.split(",")] if args.classes
